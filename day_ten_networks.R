...
set.vertex.attribute(n, "prestige", prestige(n, cmode="eigenvector"))
??prestige


#get a dataframe of those
Q<-get.vertex.attribute(n, "prestige")
V<-get.vertex.attribute(n, "between")
R<-get.vertex.attribute(n, "games")
S<-get.vertex.attribute(n, "vertex.names")
...
ggplot(XXXXXX, aes(x, y, xend = xend, yend = yend)) +
  #change the GEOMS
  geom_edges(color = "orange") +
  geom_nodes(aes(color = games)) +
  #and always run theme blank
